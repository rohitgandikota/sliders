...
def train(
    ...
    flush()
    ### EYE SLIDER
    folder_main = '/share/u/rohit/eye_size'
    folder_to_look = 'high'
    scale_to_look = 3
    
    ### LIGHT SLIDER
    folder_main = '/share/u/rohit/lighting_data'
    folder_to_look = 'inputs'
    scale_to_look = 3
...
#                 1, 25, (1,)
            ...
            if folder_main == '/share/u/rohit/eye_size':
                ims = os.listdir(f'{folder_main}/{folder_to_look}/')
                ims = [im_ for im_ in ims if '.png' in im_]
                random_sampler = random.randint(0, len(ims)-1)
                array = np.array(Image.open(f'{folder_main}/{folder_to_look}/{ims[random_sampler]}'))
                half = array.shape[1]//2
                img1 = Image.fromarray(array[:,:half,:]).resize((512,512))
                img2 = Image.fromarray(array[:,half:,:]).resize((512,512))
    #             img_src = Image.open(f'{folder_main}/src/{ims[random_sampler]}').resize((512,512))

                if folder_to_look == 'high':
                    folder_to_look = 'low'
                    scale_to_look = 1
                else:
                    folder_to_look = 'high'
                ...
                img1 = Image.open(f'{folder_main}/{folder_to_look}/{ims[random_sampler]}')
                img2 = Image.open(f'{folder_main}/outputs/{ims[random_sampler]}')
            ...
            denoised_latents_high, high_noise = train_util.get_noisy_image(
                ...
                noise_scheduler,
                start_timesteps=0,
                total_timesteps=timesteps_to+1)
