...
if __name__=='__main__':
    parser = argparse.ArgumentParser(
                    prog = 'Generate Custom Diffusion Images',)
    ...
    pipe = CustomDiffusionXLPipeline.from_pretrained('stabilityai/stable-diffusion-xl-base-1.0', torch_dtype=torch.float16).to("cuda")
    ...
    file = model_id.split('/')[-2]

    os.makedirs(f'/share/u/rohit/www/customdiffusion/{file}/',exist_ok=True)
    for idx,prompt in enumerate(prompts):
        ...
        for i, im in enumerate(images):
            im.save(f'/share/u/rohit/www/customdiffusion/{file}/{case_number}_{i}.png')